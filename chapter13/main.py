...
# Load the ONNX model
sess_10 = rt.InferenceSession("acquisition_model_10_onnx", providers=["CPUExecutionProvider"])
sess_50 = rt.InferenceSession("acquisition_model_50_onnx", providers=["CPUExecutionProvider"])
sess_90 = rt.InferenceSession("acquisition_model_90_onnx", providers=["CPUExecutionProvider"])
...
def predict(features: FantasyAcquisitionFeatures):
    ...
    input_data = np.array([[features.waiver_value_tier, features.fantasy_regular_season_weeks_remaining,
                             features.league_budget_pct_remaining]], dtype = np.int64)
    
    # Calls onnx runtime to get inferences
